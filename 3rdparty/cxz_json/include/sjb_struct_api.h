...
// ���Ͱ�;
#define STRUCT(TYPE) inline void sjb_bind_##TYPE(cJSON* json, int m, TYPE *str, int i, int size)
...
static void sjb_bind_uint64(cJSON* json, int m, uint64_t *str, int i, int size)
{  
  if(m)
  {
...
#if 0  //zkwhite , vs not build, only use to linux
...
#else
inline int ARR_SKIP(int* sub, int tsize)
{
    int i = 0; int ret = tsize; /* bytes [from curr level to end level] */
    for (i = ARR_CURR(sub); i < (ARR_STRING(sub) ? ARR_SUBN(sub) - 1 : ARR_SUBN(sub)); i++)
    {
        ret *= sub[3 + i];
    }
